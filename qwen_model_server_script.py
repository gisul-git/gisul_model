...
# FASTAPI APP
# ----------------------------------------------------------------------------
app = FastAPI(title="Qwen Question Generation API - Production Hardened", version="6.0.0")
...
def build_coding_prompt(request_data):
    return f"""Generate a {request_data['difficulty']} difficulty coding problem about: {request_data['topic']}
Language: {request_data['language']}
Target: Mid to Senior developers

CRITICAL RULES:
- Return ONLY valid JSON (no markdown, no extra text)
- ALL string fields MUST be simple strings (NOT nested objects)
- constraints MUST be a list of strings
- examples and testCases MUST be lists of objects
- starterCode MUST be a simple string

EXACT JSON STRUCTURE (MANDATORY):
{{
  "problemStatement": "Complete problem description as a single string",
  "inputFormat": "Input format as a single string",
  "outputFormat": "Output format as a single string",
  "constraints": ["constraint 1", "constraint 2", "constraint 3"],
  "examples": [{{"input": "example input", "output": "example output", "explanation": "why"}}],
  "testCases": [{{"input": "test input", "expectedOutput": "expected output", "isHidden": false}}],
...
def build_sql_prompt(request_data):
    return f"""Generate a {request_data['difficulty']} difficulty SQL problem about: {request_data['topic']}
Database: {request_data.get('database_type', 'PostgreSQL')}

CRITICAL RULES:
- Return ONLY valid JSON
- problemStatement MUST be a simple string
- schema MUST be a valid object with tables array
- expectedQuery MUST be a simple string
- explanation MUST be a simple string

EXACT JSON STRUCTURE:
{{
  "problemStatement": "Problem description as single string",
  "schema": {{
    "tables": [{{
      "name": "users",
      "columns": [
  ...
  "expectedQuery": "SELECT * FROM users WHERE...",
...
async def root():
    return {
        ...
        "v5_fixes": [
            "BUG1: logger.info between try/except caused SyntaxError",
            "BUG2: 7-space indent on pending_results check broke poll loop",
            "BUG3: Missing \\s* in output-of regex broke expression extraction",
            "BUG4: Raw RuntimeError stored in pending_results caused 500 errors"
        ]
    }
...
@app.post("/api/v1/generate-topics", response_model=TopicGenerationResponse)
async def generate_topics(request: TopicGenerationRequest):
    update_stats("topics")
    cache_key = generate_cache_key("topics", request.model_dump())
    if request.use_cache:
        cached = get_from_cache(cache_key)
        if cached:
            cached["cache_hit"] = True
            return TopicGenerationResponse(**cached)
    try:
        result = await add_to_batch_and_wait("topics", request.model_dump(), cache_key, build_topics_prompt, 3000)
        return TopicGenerationResponse(**result)
    except Exception as e:
        STATS["errors"] += 1
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/v1/generate-mcq", response_model=MCQGenerationResponse)
async def generate_mcq(request: MCQGenerationRequest):
    update_stats("mcq")
    data = request.model_dump()
    if not data.get("request_id"):
        data["request_id"] = str(uuid.uuid4())
    cache_key = generate_cache_key("mcq", data)
    if request.use_cache and cache_key in RESPONSE_CACHE:
        cached = RESPONSE_CACHE[cache_key]
        cached["cache_hit"] = True
        return MCQGenerationResponse(**cached)
    result = await enqueue_and_wait(data, cache_key)
    return MCQGenerationResponse(**result)


@app.post("/api/v1/generate-subjective", response_model=SubjectiveGenerationResponse)
async def generate_subjective(request: SubjectiveGenerationRequest):
    update_stats("subjective")
    cache_key = generate_cache_key("subjective", request.model_dump())
    if request.use_cache:
        cached = get_from_cache(cache_key)
        if cached:
            cached["cache_hit"] = True
            return SubjectiveGenerationResponse(**cached)
    try:
        result = await add_to_batch_and_wait("subjective", request.model_dump(), cache_key, build_subjective_prompt, 2000)
        return SubjectiveGenerationResponse(**result)
    except Exception as e:
        STATS["errors"] += 1
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/v1/generate-coding", response_model=CodingGenerationResponse)
async def generate_coding(request: CodingGenerationRequest):
    update_stats("coding")
    cache_key = generate_cache_key("coding", request.model_dump())
    if request.use_cache:
        cached = get_from_cache(cache_key)
        if cached:
            cached["cache_hit"] = True
            return CodingGenerationResponse(**cached)
    try:
        result = await add_to_batch_and_wait("coding", request.model_dump(), cache_key, build_coding_prompt, 2500)
        return CodingGenerationResponse(**result)
    except Exception as e:
        STATS["errors"] += 1
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/v1/generate-sql", response_model=SQLGenerationResponse)
async def generate_sql(request: SQLGenerationRequest):
    update_stats("sql")
    cache_key = generate_cache_key("sql", request.model_dump())
    if request.use_cache:
        cached = get_from_cache(cache_key)
        if cached:
            cached["cache_hit"] = True
            return SQLGenerationResponse(**cached)
    try:
        result = await add_to_batch_and_wait("sql", request.model_dump(), cache_key, build_sql_prompt, 1500)
...
@app.post("/api/v1/generate-aiml", response_model=AIMLGenerationResponse)
async def generate_aiml(request: AIMLGenerationRequest):
    update_stats("aiml")
    cache_key = generate_cache_key("aiml", request.model_dump())
    if request.use_cache:
        cached = get_from_cache(cache_key)
        if cached:
            cached["cache_hit"] = True
            return AIMLGenerationResponse(**cached)
    try:
        token_limit = calculate_aiml_token_limit(request.model_dump())
        result = await add_to_batch_and_wait("aiml", request.model_dump(), cache_key, build_aiml_prompt, token_limit)
        return AIMLGenerationResponse(**result)
    except ValueError as e:
        STATS["errors"] += 1
