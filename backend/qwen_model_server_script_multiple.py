...
# ── Domain rotation pool — used to ensure variety across Generate clicks ──────
_AIML_DOMAIN_POOL = [
    "telecom company",
    "e-commerce and online retail",
    "healthcare and hospitals",
    "banking and financial services",
    "manufacturing and IoT sensors",
...
# Tracks which domain index to use next, keyed by (topic, difficulty)
_AIML_DOMAIN_INDEX: dict = {}


def build_aiml_prompt(request_data):
    """
    Pass 1 prompt — model generates SCHEMA + TASKS only (no data rows).
    Data rows are generated programmatically in Pass 2 by generate_aiml_dataset().

    Domain rotation ensures each Generate click produces a different industry angle.
    Tasks are fully generated by the model — never hardcoded — so they reflect
    the actual features, target, and problem type of this specific dataset.
    """
    topic     = request_data['topic']
    difficulty = request_data['difficulty']
    q_index   = request_data.get('question_index', 0)

    # Pick domain — rotate per (topic, difficulty) key so every click differs
    pool_key = f"{topic.lower()}_{difficulty.lower()}"
    current_idx = _AIML_DOMAIN_INDEX.get(pool_key, 0)
    domain = _AIML_DOMAIN_POOL[(current_idx + q_index) % len(_AIML_DOMAIN_POOL)]
    # Advance index for next Generate click on same topic
    _AIML_DOMAIN_INDEX[pool_key] = (current_idx + q_index + 1) % len(_AIML_DOMAIN_POOL)

    return f"""You are an expert AI/ML assessment designer.

Generate a {difficulty} difficulty AI/ML dataset problem about: {topic}
Industry context: {domain}

DATASET RULES:
- Choose 6-12 feature names that are SPECIFIC and REALISTIC for this topic in the {domain} context.
- feature_types format — use EXACTLY one of:
    numerical (continuous, range: X to Y)   ← pure numbers only, NO units
    numerical (integer, range: X to Y)      ← pure numbers only, NO units
    categorical (values: A, B, C)
- target variable MUST NOT appear in features list.
- Do NOT generate any data rows.

TASKS RULES:
- Generate EXACTLY 5 tasks specific to THIS problem.
- Every task must mention actual column names from YOUR dataset.
- Tasks must match YOUR problem type (regression/classification/clustering).
- Task 1: data loading steps specific to this dataset structure.
- Task 2: preprocessing steps naming YOUR specific categorical and numerical columns.
- Task 3: EDA plots that make sense for YOUR target and features.
- Task 4: recommend 2 algorithms that suit YOUR problem type and explain why.
- Task 5: model comparison using metrics that fit YOUR problem, with business insight.
- NEVER write generic tasks like "handle missing values" — always name the column.

Return ONLY valid JSON:
{{
  "problemStatement": "2-3 sentence real-world problem description specific to {domain}",
  "dataset": {{
    "description": "What this dataset represents in {domain} context",
    "features": ["col1", "col2", "col3"],
    "feature_types": {{
      "col1": "numerical (continuous, range: 20 to 150)",
      "col2": "categorical (values: A, B, C)"
    }},
    "target": "target_column_name",
    "target_type": "binary (0: label0, 1: label1) OR continuous OR multiclass (classes: A, B, C)",
    "class_distribution": {{"label0": 75, "label1": 25}},
    "size": "150 samples"
  }},
  "tasks": [
    "Task 1: <specific loading and exploration task mentioning actual column names>",
    "Task 2: <specific preprocessing task naming which columns to encode/scale and how>",
    "Task 3: <specific EDA task naming which plots and which columns to visualize>",
    "Task 4: <specific model training task naming 2 algorithms suited for this problem and why>",
    "Task 5: <specific comparison task naming metrics relevant to this problem type and business insight>"
  ],
  "preprocessing_requirements": [
    "<specific step 1 naming actual columns>",
    "<specific step 2 naming actual columns>",
    "<specific step 3 e.g. class imbalance handling if applicable>"
  ],
  "expectedApproach": "Name 2 specific algorithms for this problem and explain WHY they suit it.",
  "evaluationCriteria": ["<metric1 suited for this problem>", "<metric2>", "<metric3>"],
  "difficulty": "{difficulty}"
}}
...
async def generate_aiml(request: AIMLGenerationRequest):
    ...
    async def _aiml_task():
        ...
        try:
            ...
            for i in range(num_q):
                item_data = {k: v for k, v in data.items() if k not in ("num_questions",)}
                item_data["question_index"] = i  # used by domain rotation in build_aiml_prompt
